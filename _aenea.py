...
def reload_code():
    ...
    script_dir = "D:\\aenea-grammar-simple"
    ...
    for file in os.listdir(script_dir):
        full_file = os.path.join(script_dir, file)
        if (os.path.isfile(full_file)):
            shutil.copy(full_file, macro_dir)
...
class ReloadGrammarsRule(dragonfly.MappingRule):
    mapping = {command_table['force reload grammars']: dragonfly.Function(reload_code)}
